...
/* ── Types ──────────────────────────────────────────────────────────────── */
interface UserBrief { id: string; nom: string; photo?: string; }

interface GroupMessage {
  id: string; contenu: string; createdAt: string;
  user: UserBrief;
  reads: { id: string; user: UserBrief }[];
}
...
const Av: React.FC<{ user: UserBrief; size?: number; style?: React.CSSProperties }> = ({ user, size = 32, style }) => (
  <div style={{
    width: size, height: size, borderRadius: '50%', background: col(user.id), flexShrink: 0,
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    fontSize: size * 0.38, fontWeight: 700, color: '#fff', overflow: 'hidden', ...style,
  }}>
    {user.photo ? <img src={user.photo} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : user.nom[0]?.toUpperCase()}
  </div>
);
...
const UserPicker: React.FC<{ users: UserBrief[]; onSelect: (u: UserBrief) => void; onClose: () => void }> = ({ users, onSelect, onClose }) => {
  ...
  const filtered = users.filter(u => u.nom.toLowerCase().includes(q.toLowerCase()));
  return (
        ...
        <div style={{ maxHeight: 280, overflowY: 'auto' }}>
          ...
          {filtered.map(u => (
            <div key={u.id} onClick={() => onSelect(u)}
              style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 18px', cursor: 'pointer', transition: 'background 0.1s' }}
              onMouseEnter={e => (e.currentTarget.style.background = '#F5F7FF')}
              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
            >
              <Av user={u} size={34} />
              <span style={{ fontWeight: 600, fontSize: 13.5, color: '#1A1D2E' }}>{u.nom}</span>
            </div>
          ))}
        </div>
        <div style={{ padding: '10px 18px', borderTop: '1px solid #F5F7FA' }}>
          <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 12, color: '#9CA3AF', cursor: 'pointer', fontWeight: 500 }}>Annuler</button>
        </div>
      </div>
    </div>
  );
...
const MessagesPage: React.FC = () => {
  ...
  /* Load all users + connect socket once */
  useEffect(() => {
    apiService.getUsers().then(r => {
      const others = (r.data as any[]).filter(u => u.id !== me?.id);
      setAllUsers(others);
    }).catch(() => {});
  ...
  const fetchConversations = () => {
    apiService.getDmConversations().then(r => setConversations(r.data)).catch(() => {});
  };
  ...
  useEffect(() => {
    const handler = (msg: DmMessage) => {
      const partnerId = msg.sender.id === me?.id
  ...
  /* Fallback sync poll (30 s) — catches missed WS events */
  useEffect(() => {
    ...
    tick(); // immediate load on view change
    const id = setInterval(tick, 30000);
    return () => clearInterval(id);
  }, [view]);

  /* Send — WebSocket will push the message back, no manual refetch needed */
  const handleSend = async () => {
    ...
    try {
      if (view === 'group') {
        ...
        await apiService.sendDmMessage(view, text);
        fetchConversations();
      }
    } catch {}
    setSending(false);
  ...
  const startDm = (user: UserBrief) => {
    ...
    if (!conversations.find(c => c.partner.id === user.id)) {
      setConversations(prev => [{
        partner: user,
        lastMessage: { contenu: '', createdAt: new Date().toISOString(), senderId: '' },
        unread: 0,
      }, ...prev]);
    }
  };
          ...
          <div style={{ padding: '14px 10px 8px' }}>
            ...
            <div
              ...
              style={{
                ...
                background: view === 'group' ? '#EEF2FF' : 'transparent',
          ...
          <div style={{ padding: '4px 10px 6px', flex: 1, overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '4px 0 6px 6px' }}>
              ...
              <button
                ...
                style={{ width: 20, height: 20, borderRadius: 6, border: '1px solid #E5E7EB', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#6B7280', lineHeight: 1 }}
              >+</button>
            </div>

            {conversations.length === 0 && (
              <p style={{ fontSize: 11, color: '#D1D5DB', textAlign: 'center', marginTop: 12, fontWeight: 500 }}>Aucune conversation</p>
            )}
            ...
            {conversations.map(conv => {
              ...
              return (
                <div
                  key={conv.partner.id}
                  onClick={() => setView(conv.partner.id)}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 9, padding: '8px 10px', borderRadius: 10,
                    cursor: 'pointer', background: isActive ? '#EEF2FF' : 'transparent', transition: 'background 0.12s',
                    position: 'relative',
                  }}
                  ...
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    ...
                    {conv.unread > 0 && (
                      <span style={{
                        ...
                        padding: '0 3px', border: '1.5px solid #FAFBFF',
                      }}>{conv.unread > 9 ? '9+' : conv.unread}</span>
                    )}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 12.5, fontWeight: conv.unread > 0 ? 700 : 600, color: isActive ? '#4F46E5' : '#1A1D2E', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {conv.partner.nom}
                    </p>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#fff', minWidth: 0 }}>
          ...
          <div style={{ padding: '13px 20px', borderBottom: '1px solid #F0F2F8', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
            {view === 'group' ? (
              ...
              <>
                ...
                <div>
                  <p style={{ margin: 0, fontWeight: 700, fontSize: 15, color: '#1A1D2E' }}>{activePartner.nom}</p>
            ...
            {groups.map(group => (
              <div key={group.date}>
                ...
                {group.msgs.map((msg, idx) => {
                  ...
                  return (
                    <div key={msg.id} style={{ display: 'flex', flexDirection: isMine ? 'row-reverse' : 'row', alignItems: 'flex-start', gap: 9, marginBottom: 5, marginLeft: isGrouped ? 41 : 0 }}>
                      {!isMine && (
                      ...
                      <div style={{ maxWidth: '65%', display: 'flex', flexDirection: 'column', alignItems: isMine ? 'flex-end' : 'flex-start' }}>
                        {!isMine && showAvatar && (
                          <p style={{ margin: '0 2px 3px', fontSize: 11, fontWeight: 700, color: col(senderId) }}>{senderUser.nom}</p>
                        )}
                        <div style={{
                          padding: '9px 13px',
          ...
          <div style={{ padding: '11px 16px', borderTop: '1px solid #F0F2F8', display: 'flex', gap: 10, alignItems: 'flex-end', flexShrink: 0 }}>
            ...
            <textarea
              ...
              onKeyDown={handleKey}
              placeholder={view === 'group' ? "Écrire au canal… (Entrée pour envoyer)" : `Message à ${activePartner?.nom || ''}…`}
              maxLength={1000}
              rows={1}
              style={{
                flex: 1, resize: 'none', padding: '10px 14px', borderRadius: 12,
                border: '1.5px solid #EEF0F6', background: '#F8FAFC',
                fontSize: 13.5, fontWeight: 500, color: '#1A1D2E', outline: 'none',
            ...
            <button
              ...
              style={{
                ...
                background: input.trim() ? '#4F46E5' : '#EEF2FF', border: 'none',
