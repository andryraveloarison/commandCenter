...
const CreateProjectModal: React.FC<Props> = ({ open, onClose, users, onSuccess }) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      onSuccess(); onClose(); setForm(EMPTY_FORM); reset();
    },
  });
      ...
      <div className="bg-white w-full max-w-xl shadow-2xl overflow-y-auto" style={{ borderRadius: 24, maxHeight: '90vh' }}>
        ...
        <form onSubmit={e => { e.preventDefault(); mutation.mutate(); }} className="px-8 pt-5 pb-8 space-y-5">
          ...
          <div>
            ...
            <input type="text" required value={form.nom} onChange={e => { setForm(f => ({ ...f, nom: e.target.value })); reset(); }} placeholder="EX: MIGRATION CLOUD 2026" className="input-clean" />
          </div>

          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5">Description</label>
            <textarea rows={2} value={form.description} onChange={e => { setForm(f => ({ ...f, description: e.target.value })); reset(); }} placeholder="Spécifications et objectifs..." className="input-clean resize-none" />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5">Date de Lancement</label>
              <input type="date" required value={form.dateDebut} onChange={e => setForm(f => ({ ...f, dateDebut: e.target.value }))} className="input-clean" />
            </div>
            <div>
          ...
          <div>
            ...
            <div className="max-h-32 overflow-y-auto rounded-xl p-2 space-y-0.5" style={{ background: 'var(--bg-app)' }}>
              {users.length === 0 && <p className="text-[10px] text-slate-300 font-bold uppercase text-center py-2">Aucun utilisateur disponible</p>}
              {users.map(u => (
                <label key={u.id} className="flex items-center gap-3 px-2 py-1.5 rounded-xl hover:bg-white cursor-pointer transition-colors">
                  <input type="checkbox" checked={form.teamUserIds.includes(u.id)} onChange={() => toggleMember(u.id)} className="w-4 h-4 accent-slate-900 rounded flex-shrink-0" />
